...
function speciesDisplayName(item) {
  return item.common_name || item.name || 'Unknown species';
}

function getRecordsCount(item) {
  return Number(item.observations_count || 0) + Number(item.identifications_count || 0);
}
    ...
    <article class="overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/5 hover-lift">
      ...
      <div class="p-5">
        <div class="flex items-start justify-between gap-3">
          <div>
            <h3 class="text-xl font-bold">${speciesDisplayName(item)}</h3>
            <p class="text-sm italic text-slate-400">${item.scientific_name || ''}</p>
          </div>
...
function renderLists() {
  ...
  observationsList.innerHTML = state.observations.length ? state.observations.slice().reverse().map(obs => `
    <article class="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div class="flex items-start justify-between gap-3">
        <div>
          <h4 class="font-semibold">${obs.common_name || obs.scientific_name || 'Observation'}</h4>
          <p class="text-xs text-slate-400">${obs.region_name || 'Unknown region'} • ${obs.observation_date || 'No date'} • ${obs.observer || 'Anonymous'}</p>
        </div>
  ...
  identificationsList.innerHTML = state.identifications.length ? state.identifications.slice().reverse().map(item => `
    <article class="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div class="flex items-start justify-between gap-3">
        <div>
          <h4 class="font-semibold">${item.identified_common_name || 'Identification'}</h4>
          <p class="text-xs text-slate-400">${item.identified_scientific_name || ''}</p>
        </div>
        <span class="record-pill">${item.confidence ? Math.round(Number(item.confidence) * 100) + '%' : '—'}</span>
      </div>
    ...
    <article class="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div class="flex items-start justify-between gap-3">
        <div>
          ...
          <p class="text-xs text-slate-400">${doc.document_type || 'Document'} • ${doc.author || 'Unknown author'}</p>
        </div>
        ${doc.file_path ? `<span class="record-pill">${doc.file_path.split('/').pop()}</span>` : ''}
      </div>
      ${doc.description ? `<p class="mt-3 text-sm text-slate-300">${doc.description}</p>` : ''}
      ${doc.file_path ? `<div class="mt-3"><a class="btn-secondary px-4 py-2" href="http://localhost:3000/${doc.file_path}" target="_blank" rel="noreferrer" type="button">Open file</a></div>` : ''}
    </article>
