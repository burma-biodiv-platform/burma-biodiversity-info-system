...
// Ensure local file uploading storage directory exists
if (!fs.existsSync(UPLOAD_DIR)) {
  fs.mkdirSync(UPLOAD_DIR, { recursive: true });
}

// Open and establish connection to SQLite database
const db = new sqlite3.Database(DB_PATH, (err) => {
  if (err) {
    ...
    db.run("PRAGMA foreign_keys = ON", (fkErr) => {
      if (fkErr) {
        console.error("Failed to enable SQLite foreign keys:", fkErr.message);
      } else {
...
// Promise-based execution wrappers to resolve unhandled promise rejections
...
// Configure Multer for processing file attachments safely
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (_req, file, cb) => {
      // Remove any unsafe characters from original filename
      const safeName = file.originalname.replace(/[^\w.\-]+/g, "_");
      cb(null, `${Date.now()}_${safeName}`);
    }
  }),
  limits: { fileSize: 25 * 1024 * 1024 } // Maximum: 25 Megabytes
});
...
// Create schema structures and seed initial datasets if empty
async function initializeDatabaseSchema() {
  try {
    ...
    // 5. Create Portal Modules Table (to support standard dashboard queries)
    await run(`
    ...
    if (moduleCount.count === 0 || !moduleCount.count) {
      ...
      for (const modName of defaultModules) {
        await run("INSERT OR IGNORE INTO modules (name) VALUES (?)", [modName]);
      }
      console.log("Seeded default portal modules.");
    }
    ...
    // Seed bird taxonomy database if empty
    ...
    if (taxonCount.count === 0 || !taxonCount.count) {
      ...
      console.log("Seeded default bird taxonomy records.");
    }
  } catch (err) {
    console.error("Critical database schema/seeding error:", err);
  }
}

// API ROUTING ENDPOINTS

// GET: Fetch all catalogued species
...
// POST: Register a new species profile
app.post("/api/v1/species", async (req, res) => {
  try {
    ...
    if (!common_name || !scientific_name) {
      return res.status(400).json({ error: "Required fields missing: common_name and scientific_name." });
    }
    ...
    res.status(201).json({ id: result.lastID, message: "Taxonomy profile created successfully." });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// GET: Fetch Myanmar administrative regions
...
// POST: Add new field observation (with photo upload support)
app.post("/api/v1/observations", upload.single("photo"), async (req, res) => {
  try {
    ...
    if (!taxon_id || !region_id || !observation_date) {
      return res.status(400).json({ error: "Missing required values: taxon_id, region_id, and observation_date." });
    }
    ...
    res.status(201).json({ id: result.lastID, message: "Field observation registered successfully." });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// GET: Fetch uploaded reference documents
...
// POST: Upload a literature document (e.g., pdf/images checklist)
app.post("/api/v1/documents", upload.single("file"), async (req, res) => {
  try {
    ...
    res.status(201).json({ id: result.lastID, message: "Document added to archival system successfully." });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// GET: Search unified database index
...
// GET: Analytics Statistics for Dashboard rendering
app.get("/api/v1/dashboard", async (_req, res) => {
  try {
    ...
    const recentSpecies = await all(`
      SELECT id, common_name, scientific_name, iucn_status, family 
      FROM taxonomy 
    ...
    const latestDocuments = await all(`
...
app.use((err, _req, res, _next) => {
  console.error("Caught Server Exception:", err.stack);
